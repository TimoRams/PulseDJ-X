...
void MenuBar::setupMenus() {
    ...
    viewMenu->addAction("Minimize to Tray")->setEnabled(false);
    
    // Tools menu (placeholder for future features)
    toolsMenu = addMenu("Tools");
    toolsMenu->addAction("Audio Settings")->setEnabled(false);
    toolsMenu->addAction("MIDI Configuration")->setEnabled(false);
    toolsMenu->addAction("Plugin Manager")->setEnabled(false);
...
void MenuBar::exportSettings() {
    ...
    if (!fileName.isEmpty()) {
        ...
        foreach (const QString& group, groups) {
            ...
            foreach (const QString& key, config.allKeys()) {
                ...
                if (value.type() == QVariant::Color) {
                    groupObj[key] = value.value<QColor>().name();
                } else {
        ...
        if (file.open(QIODevice::WriteOnly)) {
            ...
            QMessageBox::critical(this, "Export Failed", "Failed to write settings file.");
        }
    }
}
                ...
                if (QMessageBox::question(this, "Import Settings", 
                    ...
                    for (auto it = jsonObj.begin(); it != jsonObj.end(); ++it) {
                        ...
                        for (auto groupIt = groupObj.begin(); groupIt != groupObj.end(); ++groupIt) {
                            config.setValue(groupIt.key(), groupIt.value().toVariant());
                        }
                    ...
                    QMessageBox::information(this, "Import Successful", 
                                           "Settings imported successfully. Restart recommended.");
                }
            } else {
                QMessageBox::critical(this, "Import Failed", 
                                    QString("Invalid JSON file: %1").arg(error.errorString()));
            }
        } else {
            QMessageBox::critical(this, "Import Failed", "Failed to read settings file.");
        }
    }
}
...
void MenuBar::resetSettings() {
    ...
    if (ret == QMessageBox::Yes) {
        ...
        // Clear deck settings as well - Note: This method might need to be implemented
        // DeckSettings::instance().resetToDefaults();
        
        QMessageBox::information(this, "Settings Reset", 
                               "All settings have been reset to defaults. Restart recommended.");
    }
}

void MenuBar::showAbout() {
    QMessageBox aboutBox(this);
    aboutBox.setWindowTitle("About BetaPulseX");
    aboutBox.setTextFormat(Qt::RichText);
    aboutBox.setText(
        "<h3>BetaPulseX</h3>"
        "<p>Professional DJ Software</p>"
        "<p><b>Version:</b> 1.0-beta</p>"
        "<p><b>Built with:</b> Qt6 & JUCE Framework</p>"
        "<br>"
        "<p><b>Features:</b></p>"
        "<ul>"
        "<li>Dual deck mixing with crossfader</li>"
        "<li>Real-time waveform display</li>"
        "<li>BPM detection and sync</li>"
        "<li>Performance pads</li>"
        "<li>Professional audio effects</li>"
        "</ul>"
        "<br>"
        "<p><b>Copyright © 2024 BetaPulseX Project</b></p>"
